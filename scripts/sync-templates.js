...
function generateSourceRegistry(templates) {
  const entries = templates.map(t => {
    const pagePath = path.join(TEMPLATES_DIR, t.dirName, 'page.tsx');
  ...
  return `/**
 * AUTO-GENERATED by scripts/sync-templates.js — do not edit.
 * Maps template hrefs to their raw source code.
 */
...
async function main() {
  ...
  fs.writeFileSync(sourceRegistryPath, generateSourceRegistry(templates));
